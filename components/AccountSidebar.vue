<template>
  <div class="absolute left-[84px] top-[32px] w-[205px] flex flex-col gap-[30px] text-black z-10">
    <!-- Active Item (My Account by default) -->
    <div class="flex items-center justify-between">
      <div class="text-[16px] leading-[27.82px] uppercase font-medium">{{ activeItem }}</div>
      <UIcon name="i-heroicons-chevron-right" class="w-[25px] h-[25px] text-black" />
    </div>
    ...
    <div v-for="menuItem in menuItems" :key="menuItem.name" 
      ...
      <div class="text-[16px] leading-[27.82px] uppercase font-light">{{ menuItem.name }}</div>
      <UIcon name="i-heroicons-chevron-right" class="w-[25px] h-[25px] text-black" />
    </div>
    
    <!-- Order Upload (conditional) -->
    <div v-if="showOrderUpload" 
         class="flex items-center justify-between cursor-pointer hover:opacity-70 transition-opacity" 
         @click="handleOrderUploadClick">
      <div class="text-[16px] leading-[27.82px] uppercase font-light">Order Upload</div>
      <UIcon name="i-heroicons-chevron-right" class="w-[25px] h-[25px] text-black" />
    </div>
  </div>
</template>
