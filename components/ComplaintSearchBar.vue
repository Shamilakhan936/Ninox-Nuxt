...
<template>
  <div class="flex flex-col md:flex-row justify-between gap-20 mb-[90px]">
    ...
    <div class="w-full md:w-[40%]">
      <div
        class="flex items-center w-full rounded-full "
      >
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Complaint Number/Article Number"
          class="w-full px-9 py-3 bg-transparent focus:outline-none text-sm placeholder-[#6F6259] w-full rounded-full border border-[#C9C7C5] bg-white text-[#6F6259]"
    ...
    <div class="w-full md:w-[40%] relative">
      <div
        ...
        class="flex items-center rounded-full border border border-[#C9C7C5] bg-white px-4 py-3 gap-2 text-[#6F6259] w-full text-sm cursor-pointer hover:border-[#B8B0A8] transition-colors"
      >
        <ChevronLeftIcon @click.stop="previousMonth" class="w-4 h-4 cursor-pointer hover:text-[#4A3F35] flex-shrink-0" />
        <span class="flex-1 text-center truncate">{{ formattedRange }}</span>
        <ChevronRightIcon @click.stop="nextMonth" class="w-4 h-4 cursor-pointer hover:text-[#4A3F35] flex-shrink-0" />
      </div>

    
      <div v-if="showDatePicker" class="absolute top-full mt-2 bg-white border border border-[#C9C7C5] rounded-lg shadow-lg z-50 p-4 min-w-[300px] w-full max-w-[350px] left-0 right-0 mx-auto">
        ...
        <div class="grid grid-cols-7 gap-1 mb-4">
          <div v-for="day in ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']" :key="day" class="text-xs text-center text-gray-500 py-1">
            {{ day }}
          </div>
          <div
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-gray-600 gap-2">
          <div>
            <span class="font-medium">Start:</span> {{ format(startDate, 'dd MMM yyyy') }}
          </div>
          <div>
            <span class="font-medium">End:</span> {{ format(endDate, 'dd MMM yyyy') }}
          </div>
        </div>
...
const formattedRange = computed(() =>
  `${format(startDate.value, 'dd MMM yyyy')} – ${format(endDate.value, 'dd MMM yyyy')}`
)
...
const isSelectedDate = (date) => {
  return format(date, 'yyyy-MM-dd') === format(startDate.value, 'yyyy-MM-dd') ||
