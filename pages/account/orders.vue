<template>
  <div
    class="min-h-screen table-orders-container"
        ...
        <div class="ml-[400px] max-w-[950px]">
          ...
          <div class="mb-[64px]">
            ...
            <p class="text-[16px] leading-[24px] text-[#6f6259] max-w-[869px]">
              View and manage all your orders. You can search by order number,
              filter by date range, and track the status of each order. Click
              "View" to see detailed information about any order.
            </p>
          </div>
          ...
          <div class="flex flex-wrap gap-6 mb-8">
            ...
            <div class="flex flex-col gap-2">
              <label
                class="text-[16px] leading-[14px] uppercase tracking-[0.2em] font-light text-[#3d3935]"
              >
                Order No.
              </label>
              <input
          ...
          <div class="mb-[120px]">
            <h2 class="text-[30px] font-light text-[#2d1713] mb-[64px]">
              Recent Orders
            </h2>
            ...
            >
              <!-- Table Header -->
              <div
                class="grid grid-cols-6 gap-4 p-4 bg-gray-50 border-b text-[16px] leading-[14px] uppercase tracking-[0.2em] font-light text-[#3d3935]"
              >
                <div>Order #</div>
                <div>Order Date</div>
                <div>Status</div>
            ...
            >
              ...
              >
                ...
                {{
                  Math.min(
                    ordersPerPage,
                    filteredOrders.length - currentPage * ordersPerPage
                  )
                }}
                Orders
              </button>
      ...
      <UCard v-if="selectedOrder">
          ...
          <div v-if="selectedOrder.shipping">
            ...
            <div class="text-sm space-y-2">
              ...
              <div v-if="selectedOrder.shipping.estimatedDate">
                ...
                <span class="ml-2">{{
                  formatDate(selectedOrder.shipping.estimatedDate)
                }}</span>
              </div>
            </div>
          </div>
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="gray" variant="soft" @click="closeOrderModal">
              Close
            </UButton>
          </div>
        </template>
      ...
      <div class="absolute bottom-[28px] left-[84px]">
        <div class="flex items-center">
          <img
            ...
            alt="Copyright"
            class="w-[18.5px] h-[18.5px] mr-2"
          />
          <span class="text-[18px] leading-[26px] font-light"
            >2025, Crastino</span
          >
        </div>
      </div>
      ...
      <div class="absolute bottom-[34px] right-[84px]">
        <div class="flex items-center">
          <img
            ...
            alt="Location"
            class="w-[8px] h-[14px] mr-2"
          />
          <span
            class="text-[16px] leading-[14px] uppercase tracking-[0.2em] font-light"
          >
...
const filteredOrders = computed(() => {
  ...
  if (searchDateFrom.value) {
    filtered = filtered.filter(
      (order) => new Date(order.createdAt) >= new Date(searchDateFrom.value)
    );
  }

  if (searchDateTo.value) {
    filtered = filtered.filter(
      (order) => new Date(order.createdAt) <= new Date(searchDateTo.value)
    );
  }
